...
function handleNumber(socket) {
	...
	socket.on('phoneEntered', function(message) {
		...
		if ( !con.timeout) {
			...
			console.log('mobile successfully recieved', message.phone);
			socket.emit('phoneResult', {
				msg: "success"
			});
		}	else if ( con.timeout > Date.now() ){
			console.log('emitting expecting PIN error');
			socket.emit('err', {
				code: 05,
				msg: 'expecting PIN',
			...
			console.log('emitting timeout error');
			delete con.timeout;
			socket.emit('err', {
				code: 04,
				msg: 'timeout'
	...
	socket.on('pinEntered', function(message) {
		var con = connections[socket.id];
		if ( !con.timeout ) {
			console.log('emitting PIN not expected error');
			socket.emit('err', {
				code: 05,
			...
			delete con.timeout;
			console.log('emitting PIN timeout error');
			socket.emit('err', {
				code: 04,
				msg: 'timeout'
			});
			...
			if (message.pin != con.pin){
				console.log(message);
				console.log('emitting PIN verificartion error', message.pin, con.pin);
				socket.emit('err', {
					code: 02
...
function generateJwt(socket, uid, existing){
	...
	console.log('jwtGenerated is about to be fired');
